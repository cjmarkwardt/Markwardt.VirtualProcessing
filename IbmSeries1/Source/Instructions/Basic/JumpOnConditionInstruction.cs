...
{
    public class JumpOnConditionInstruction : ConditionalJumpInstruction
    {
        public JumpOnConditionInstruction(IEnumerable<Word> words)
            : base(words) { }

        public JumpOnConditionInstruction(ConditionCode condition, byte wordDisplacement)
            : base(condition, wordDisplacement) { }

        public override byte[] Export()
            => new byte[] { (byte)(((int)InstructionId.JumpOnCondition << 3) | (int)Condition), (byte)Jump.Unsigned };
        ...
        protected override IReadOnlyDictionary<ConditionCode, Test> Conditions { get; } = new Dictionary<ConditionCode, Test>()
        {
            ...
            { ConditionCode.ArithmeticallyLessOrEqual, l => (l.GetStatusFlag(LevelFlag.Overflow) && !l.GetStatusFlag(LevelFlag.Negative)) || (!l.GetStatusFlag(LevelFlag.Overflow) && l.GetStatusFlag(LevelFlag.Negative)) || (l.GetStatusFlag(LevelFlag.Zero)) },
        ...
        {
            public override InstructionId Id => InstructionId.JumpOnCondition;
